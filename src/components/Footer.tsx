...
export default function Footer() {
  const { 
    ...
    footerAddress,
    footerCopyright,
    footerHeritageTagline,
    footerCertifications,
    footerInfrastructureLabel,
    footerSystemsLabel,
        ...
        <div className="max-w-[1400px] 2xl:max-w-[1800px] mx-auto px-6 sm:px-10 md:px-16 relative z-10 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 sm:gap-20 lg:gap-12 xl:gap-16 mb-24 sm:mb-40">
            ...
            <div className="lg:col-span-4 flex flex-col gap-10 min-w-0">
              <Link href="/" className="flex items-center gap-4 sm:gap-6 w-fit group">
                <div className="w-10 h-10 sm:w-14 sm:h-14 border-2 border-[var(--color-primary)] flex items-center justify-center group-hover:bg-[var(--color-primary)] transition-all duration-500">
                  <span className="font-display font-black text-[18px] sm:text-[24px] text-white italic">{brandName.charAt(0)}</span>
                </div>
                <span className="font-display font-black text-[18px] sm:text-[20px] text-white uppercase italic tracking-tighter">{brandName}</span>
              </Link>
              ...
              <div className="flex flex-col gap-4 mt-4">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-mono text-[9px] sm:text-[10px] uppercase tracking-[0.5em] text-[var(--color-white)] font-black italic">{footerSystemStatus}</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-primary)]" />
                  <span className="font-mono text-[9px] sm:text-[10px] uppercase tracking-[0.5em] text-[var(--color-white)] font-black italic">{footerQualityIndex}</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-12 sm:gap-10 min-w-0">
              <div className="flex flex-col gap-8 sm:gap-10">
                <span className="font-mono text-[11px] text-[var(--color-primary)] uppercase tracking-[0.6em] font-black italic">{footerInfrastructureLabel}</span>
                <ul className="flex flex-col gap-5 sm:gap-6">
                  {footerNavInfrastructure.map((l) => (
                    <li key={l.name}>
              ...
              <div className="flex flex-col gap-8 sm:gap-10">
                <span className="font-mono text-[11px] text-[var(--color-primary)] uppercase tracking-[0.6em] font-black italic">{footerSystemsLabel}</span>
                <ul className="flex flex-col gap-5 sm:gap-6">
                  {footerNavSystems.map((l) => (
                    <li key={l.name}>
                      <Link href={l.href} className="font-sans font-black text-[12px] sm:text-[13px] text-[var(--color-silver)] hover:text-white hover:translate-x-3 transition-all duration-500 uppercase tracking-widest flex items-center gap-4 group">
                        <div className="w-0 h-[1.5px] bg-[var(--color-primary)] group-hover:w-4 transition-all" />
                        {l.name}
            ...
            <div className="lg:col-span-3 flex flex-col gap-8 min-w-0 overflow-hidden">
              <span className="font-mono text-[11px] text-[var(--color-primary)] uppercase tracking-[0.6em] font-black italic">{footerOperationsLabel}</span>
              <div className="flex flex-col gap-8">
                <a href={`tel:${footerPhone.replace(/\s+/g, '')}`} className="font-display text-[20px] lg:text-[24px] xl:text-[28px] text-white font-black italic hover:text-[var(--color-primary)] transition-colors tracking-tight">
                  {footerPhone}
                </a>
                <a 
                ...
                >
                  <span className="relative z-10 italic">{footerCtaText}</span>
                  <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-10 transition-opacity" />
                </a>
                <div className="flex flex-col gap-2 opacity-40">
                  <span className="font-mono text-[10px] text-white uppercase tracking-widest font-black">{footerHqLabel}</span>
                  <p className="font-sans text-[12px] sm:text-[13px] text-[var(--color-silver)] font-medium leading-relaxed italic break-words whitespace-pre-line">
                    {footerAddress}
                  </p>
                </div>
              </div>
          ...
          <div className="border-t border-white/5 pt-12 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="flex flex-col items-center md:items-start gap-2">
               <span className="font-mono text-[8px] sm:text-[9px] text-[var(--color-silver)] uppercase tracking-[0.4em] opacity-30 font-black text-center md:text-left">{footerCopyright}</span>
               <span className="font-mono text-[8px] sm:text-[9px] text-[var(--color-primary)] uppercase tracking-[0.4em] font-black italic">{footerHeritageTagline}</span>
            </div>
            
            <div className="flex items-center gap-8 sm:gap-12">
              {footerCertifications.map((cert) => (
                 <div key={cert.id} className="flex flex-wrap gap-8 sm:gap-12 items-center">
                    <div className="flex flex-col items-center md:items-end gap-1">
                       <span className="font-display text-[14px] sm:text-[16px] text-white font-black italic">{cert.name}</span>
                       <span className="font-mono text-[7px] sm:text-[8px] text-[var(--color-silver)] uppercase tracking-widest opacity-30">{cert.detail}</span>
                    </div>
                 </div>
              ))}
            </div>
